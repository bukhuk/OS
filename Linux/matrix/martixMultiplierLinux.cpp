...
struct ThreadData {
    class BlockMultiplier* multiplier;
    int row_block;
    int col_block;
};
...
public:
    ...
    static void* threadMultiplyBlock(void* arg) {
        auto* data = static_cast<ThreadData*>(arg);
        data->multiplier->multiplyBlock(data->row_block, data->col_block);
        delete data;
        return nullptr;
    }
    ...
    void multiplyMatricesParallel() {
        ...
        for (int i = 0; i < num_row_blocks; ++i) {
            for (int j = 0; j < num_col_blocks; ++j) {
                auto* data = new ThreadData{this, i, j};
                pthread_t thread;
                pthread_create(&thread, nullptr, &BlockMultiplier::threadMultiplyBlock, data);
